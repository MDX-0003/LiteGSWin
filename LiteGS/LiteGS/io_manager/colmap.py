...
def _resolve_image_source(path: str, image_dir: str, image_name: str) -> str:
    if os.path.isabs(image_dir) or os.path.isdir(image_dir):
        image_root = image_dir
    else:
        image_root = os.path.join(path, image_dir)
...
def load_frames(path:str,image_dir:str)->tuple[dict[int,PinHoleCameraInfo],list[ImageFrame]]:
    ...
    for ImgArg in cam_extrinsics.values():
        if ImgArg.camera_id in CameraInfoDict.keys():
            camera_frame=ImageFrame(ImgArg.id,ImgArg.viewtransform_rotation,ImgArg.tvec,ImgArg.camera_id,ImgArg.name,_resolve_image_source(path,image_dir,ImgArg.name),ImgArg.xys)
...
def __fetchPly(path):
    ...
    try:
        ...
        # 如果 PLY 文件中没有 RGB 颜色属性，直接为每个点生成随机颜色 (范围 [0, 1))
