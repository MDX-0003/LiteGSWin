...
def start(lp:arguments.ModelParams,op:arguments.OptimizationParams,pp:arguments.PipelineParams,dp:arguments.DensifyParams,
    ...
    # }
    time_thresholds = {}
    time_saved_flags = {key: False for key in time_thresholds.keys()}
    # ---------------------------------------------------------

    for epoch in range(start_epoch,total_epoch):

        epoch_loss = 0.0  # <--- 新增：用于统计当前 Epoch 的总 Loss
        ...
        with StatisticsHelperInst.try_start(epoch):
            for view_matrix,proj_matrix,frustumplane,gt_image,idx_tensor in train_loader:
                ...
                epoch_loss += loss.item()  # <--- 新增：累加单步迭代的 Loss
        ...
        # 计算当前 Epoch 的平均 Loss 并实时刷新到 tqdm 进度条右侧
        avg_loss = epoch_loss / len(train_loader)  # <--- 新增
        progress_bar.set_postfix({"Loss": f"{avg_loss:.4f}"})  # <--- 新增
        ...
        for time_label, threshold_seconds in time_thresholds.items():
            if elapsed_time >= threshold_seconds and not time_saved_flags[time_label]:
                time_saved_flags[time_label] = True 
                
                # 设置保存路径，并确保目录存在以防报错
                save_path = os.path.join(lp.model_path, "point_cloud", f"time_{time_label}")
                ...
                tqdm.write(f"\n[TIME SAVE] Saved point cloud at {time_label} mark (Epoch {epoch}, {elapsed_time:.2f}s elapsed)")
        # ---------------------------------------------------------
