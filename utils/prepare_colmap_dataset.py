...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "When calibration camera parameters are available, runs point_triangulator "
            "to triangulate the scene. Otherwise runs mapper to reconstruct camera "
            "parameters and sparse point cloud from scratch."
        )
    )
    parser.add_argument(
        "-s",
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Subdirectory name under ./data (e.g. '0613'). Used for auto-detecting "
            "calibration at data/calibration/<model_sub_dir>/ when --calib_sub_dir "
            "and --calib_sparse_path are not provided."
        ),
    )
    parser.add_argument(
        "--force_no_calib",
        action="store_true",
        help=(
            "Force COLMAP mapper mode even if calibration data is detected. "
            "Ignores any existing calibration sparse model."
        ),
    )
    parser.add_argument(
        "--calibration_root",
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="Keep every Nth image from input. 1 = keep all (no stride). 3 = keep every 3rd.",
    )
    parser.add_argument(
        "--frame_stride_min_images",
        default=3,
        type=int,
        help="Minimum number of images required after stride filtering. Default: 3.",
    )
    parser.add_argument(
        "--no_gpu",
        action="store_true",
...
def ensure_raw_images(source_path: Path) -> None:
    """Guarantee raw_imgs/ exists with original images, without modifying it.

    - If raw_imgs/ already has images → do nothing (idempotent).
    - If raw_imgs/ is missing or empty → move images from frame root into it.
    - raw_imgs/ is never modified after initial population.
    """
    raw_path = source_path / "raw_imgs"

    if raw_path.exists() and any(is_image(p) for p in raw_path.iterdir()):
        return  # already initialised — leave untouched
    ...
    for image_path in sorted(source_path.iterdir()):
        if not is_image(image_path):
            continue
        target = raw_path / image_path.name
        if target.exists():
            raise FileExistsError(f"Image already exists in raw_imgs: {target}")
        shutil.move(str(image_path), str(target))
        moved_count += 1

    if moved_count:
        logging.info("Initialised raw_imgs/ with %d image(s) from frame root.", moved_count)
    elif not any(is_image(p) for p in raw_path.iterdir()):
        raise FileNotFoundError(
            f"No images found in frame root or raw_imgs/ for: {source_path}"
        )


def ensure_input_images(source_path: Path, stride: int = 1, min_images: int = 3) -> None:
    """Populate input/ from raw_imgs/ with optional stride sampling.

    Always clears input/ first, then copies images from raw_imgs/ applying
    stride filtering at copy time.  raw_imgs/ is never modified.
    """
    ensure_raw_images(source_path)

    raw_path = source_path / "raw_imgs"
    input_path = source_path / "input"

    # Clear any existing input/ contents
    if input_path.exists():
        for item in list(input_path.iterdir()):
            if item.is_file():
                item.unlink()
            elif item.is_dir():
    ...
    raw_images = sorted(p for p in raw_path.iterdir() if is_image(p))
    before_count = len(raw_images)

    # Copy with stride: keep raw_images[0], raw_images[stride], raw_images[2*stride], ...
    copied = 0
    for i, img in enumerate(raw_images):
        if i % stride != 0:
            continue
        shutil.copy2(str(img), str(input_path / img.name))
        copied += 1

    if stride > 1:
        logging.info(
            "Frame stride %d: %d image(s) in raw_imgs/ → %d image(s) copied to input/.",
            stride,
            before_count,
            copied,
        )
    ...
    if copied < min_images:
        raise ValueError(
            f"After stride filtering only {copied} image(s) remain "
            f"(stride={stride}, min={min_images}). "
            f"Use --frame_stride_min_images to lower the threshold or reduce --frame_stride."
        )
...
def resolve_calibration_model_path(
    calibration_root: Path, model_sub_dir: str
) -> Path:
    """Resolve calibration sparse model path by model_sub_dir (auto-detect)."""
    calibration_path = calibration_root / model_sub_dir
    sparse_paths = [
        calibration_path / "sparse_calib",
        calibration_path / "sparse",
    ]
    ...
    for sparse_path in sparse_paths:
        sparse_path = sparse_path.resolve()
        for candidate in [sparse_path, sparse_path / "0"]:
            checked_paths.append(candidate)
            if (candidate / "cameras.txt").exists() and (candidate / "images.txt").exists():
                points3d_path = candidate / "points3D.txt"
                if not points3d_path.exists():
...
def run_feature_extraction_no_calib(
    source_path: Path,
    colmap_executable: str,
    use_gpu: int,
) -> None:
    """Run COLMAP feature extraction and exhaustive matching without calibration."""
    input_path = source_path / "input"
    distorted_path = source_path / "distorted"
    database_path = distorted_path / "database.db"
...
def run_mapper(source_path: Path, colmap_executable: str) -> None:
    """Run COLMAP mapper to reconstruct camera parameters and sparse point cloud."""
...
def _has_calibration_data(calibration_root: Path, model_sub_dir: str) -> bool:
    """Check if calibration sparse model exists for the given sub directory."""
    sparse_paths = [
        calibration_root / model_sub_dir / "sparse_calib",
        calibration_root / model_sub_dir / "sparse",
    ]
    for sparse_path in sparse_paths:
        for candidate in [sparse_path, sparse_path / "0"]:
            if (candidate / "cameras.txt").exists() and (candidate / "images.txt").exists():
...
def main() -> int:
    ...
    try:
        ...
        ensure_input_images(source_path, args.frame_stride, args.frame_stride_min_images)

        # Determine calibration mode
        has_calib = False
        if args.force_no_calib:
            logging.info("Calibration-less mode forced via --force_no_calib.")
        elif args.calib_sub_dir or args.calib_sparse_path:
            has_calib = True
            calibration_model_path = resolve_calibration_model(args)
            logging.info("Using calibration model: %s", calibration_model_path)
        elif args.model_sub_dir:
            calibration_root = Path(args.calibration_root)
            has_calib = _has_calibration_data(calibration_root, args.model_sub_dir)
            if has_calib:
                logging.info(
                    "Auto-detected calibration data at data/calibration/%s/.",
                    args.model_sub_dir,
                )
                # Build args-like resolution for auto-detect
                calibration_model_path = resolve_calibration_model_path(
                    calibration_root, args.model_sub_dir
                )
            else:
                logging.info(
                    "No calibration data found at data/calibration/%s/. "
                    "Running COLMAP mapper (calibration-less mode).",
                    args.model_sub_dir,
                )
        else:
            raise ValueError(
                "Cannot determine calibration mode: provide --calib_sub_dir, "
                "--calib_sparse_path, --model_sub_dir, or --force_no_calib."
            )

        use_gpu = 0 if args.no_gpu else 1

        if has_calib:
            # ── Calibration path: point_triangulator ──
            ...
            # ── Calibration-less path: mapper ──
            if args.skip_matching:
                logging.warning(
                    "--skip_matching is not supported in calibration-less mode; ignoring."
                )
            run_feature_extraction_no_calib(
                source_path,
                args.colmap_executable,
                use_gpu,
            )
            run_mapper(source_path, args.colmap_executable)
