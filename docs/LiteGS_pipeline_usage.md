...
| `--skip_triangulation` | 否 | 关闭 | 跳过三角化，直接用已有 COLMAP 输出训练 |
...
```powershell
uv run pythonrun_LiteGS_pipeline.py -s ./data/0613/2026-06-13-234829 --skip_triangulation
```
...
```powershell
uv run pythonutils/triangulate_from_calibration.py -s ./data/0608/without_sword/2026-06-08-105443 --calib_sub_dir 0608
```
...
A: 下次运行可以加 `--skip_triangulation` 跳过 COLMAP 步骤，直接从训练开始。
